---
layout: post
title: 'bzoj1095 zjoi2007 hide 树链剖分'
date: 2014-08-26 16:35
tags: [数据结构,spoj]
---
150+行的树链剖分。本来是QTREE4的代码，交过来验证一下。
在spoj上T了，恐怕是常数问题。`priority_queue`+`pair`+时间戳的做法是正确的，不过常数太大了，在bzoj上跑了23s左右。
TODO：
1. ~~把STL的堆换成multiset试试。~~ 实践证明这么做没什么优点。
2. 写点剖分
3. ~~写边剖分~~ Result: 128940 kb 10444 ms
4. 写括号序列
...
void getAns(){
	int res=INF;
	for(int i=n+1;i<=cnt;i++) res=max(res,rt[i]->maxans);
...
void modify(int u){
	...
	for(int v;u;u=fa[top[u]]){
		update(u);//printf("%d:\n",u);
		for(Node *p=pos[num[u]];p;p=p->fa) p->update();//{printf("%p\n",p);p->update();}
		++Time[mapp[top[u]]];
