title: bzoj1073 Kth shortest path  Yen's algorithm
date: 2014-09-28 11:41:17
tags: [省选题,图论,bzoj]
---

经典的**Kth shortest path**问题，除了用A\*算法外，还可以用**Yen's Algorithm**来解决，具体可见[这里](http://ycool.com/post/ppswum7)。[英文维基百科](http://en.wikipedia.org/wiki/Yen's_algorithm#Example)上有漂亮的演示，有助于理解这一算法。这里就不再赘述了，具体细节可见代码。
...
struct Cell{
	...
	Cell(){}
	Cell(int _len,int _pos,Path _path) : len(_len),pos(_pos),path(_path){revPath=path;reverse(path.begin(),path.end());}
...
Cell dijkstra(int S,int T){
	...
	return Cell(d[T],tmp.size()-fork_p-1,tmp);
}
